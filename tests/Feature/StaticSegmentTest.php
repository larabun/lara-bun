...
beforeEach(function () {
    ...
    File::put($this->staticDir.'/docs/validation.seg.flight', 'PAGE-ONLY-PAYLOAD');
...
test('a client holding the layouts gets the page alone', function () {
    $response = $this->get('/docs/validation', [
        Header::X_RSC => '1',
        Header::X_RSC_SEGMENTS => 'app/layout,app/docs/layout',
    ]);

    expect($response->getContent())->toBe('PAGE-ONLY-PAYLOAD')
...
test('a client coming from a different section gets the whole document', function () {
    // Only the root layout is shared, so the section layout still has to render.
    $response = $this->get('/docs/validation', [
        Header::X_RSC => '1',
...
test('falls back to the whole document when no segment file was prerendered', function () {
    ...
    File::delete($this->staticDir.'/docs/validation.seg.flight');
