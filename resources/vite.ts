// LaraBun's Vite plugin for the RSC build.
//
// Apps compose this into their own vite.rsc.config, so the plugin list — the
// React Compiler, Tailwind, anything else — is theirs to own:
//
//   import { larabun } from 'lara-bun/vite'
//   export default defineConfig({ plugins: [larabun(), react({ compiler: true })] })
//
...
export interface LarabunOptions {
...
// Resolved once per larabun() call. One build runs in one process, so these are
...
function resolvePaths(options: LarabunOptions): void {
...
function generateEntryRsc(): string {
  ...
  return `// GENERATED by lara-bun's Vite plugin — do not edit.
...
export function larabun(options: LarabunOptions = {}): Plugin[] {
  resolvePaths(options)

  const larabunPlugin: Plugin = {
    name: 'larabun:rsc',

    config() {
      if (!existsSync(appDir)) {
        throw new Error(`[lara-bun] No app directory at ${appDir} — nothing to build.`)
      }
      ...
      if (loadingErrors.length) {
        throw new Error(
          '[lara-bun] A page that blocks before it can paint needs a loading.tsx boundary.\n\n' +
            loadingErrors.join('\n') +
    ...
    configResolved(config: ResolvedConfig) {
      ...
      if (rscAt !== -1 && jsxAt !== -1 && jsxAt < rscAt) {
        throw new Error(
          `[lara-bun] Plugin "${names[jsxAt]}" is resolved ahead of rsc(), so it would ` +
            'transform JSX before the client/server split.\n' +
            'Put larabun() first in your plugins array. If it already is, that plugin ' +
  ...
  // rsc() ships as several plugins; spreading them keeps larabun() a single
  // entry in the app's array while guaranteeing they lead.
  return [...rsc(), larabunPlugin]
}
