...
HTML);
    ...
    /**
     * Render scripts early (before streaming completes) so the browser can
     * load JS and enable SPA navigation while Suspense completions stream.
     *
     * The Flight payload is NOT included here — it's injected after streaming
     * via __rsc_resolve_payload__() which the hydration entry awaits.
     *
     * @param  list<string>  $clientChunks
     */
    public static function renderEarlyScripts(string $initialJson, array $clientChunks): HtmlString
    {
        if ($clientChunks === []) {
            return new HtmlString('');
        }
...
HMRJS;
    ...
    <script>
        ...
        window.__webpack_require__ = function(id) { return window.__RSC_MODULES__[id]; };
        window.__webpack_chunk_load__ = function() { return Promise.resolve(); };
        window.__RSC_PAYLOAD__ = new Promise(function(r) { window.__rsc_resolve_payload__ = r; });
    </script>{$chunkTags}{$hmrScript}
HTML);
    }
}
